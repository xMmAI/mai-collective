// $MAI_DIR/hooks/lib/observability.ts
...
export async function sendEventToObservability(event: ObservabilityEvent): Promise<void> {
  const dashboardUrl = process.env.MAI_OBSERVABILITY_URL || 'http://localhost:4000/events';
...
export function getSourceApp(): string {
  return process.env.MAI_SOURCE_APP || process.env.DA || 'MAI';
}
