#!/usr/bin/env bun
// $MAI_DIR/hooks/initialize-session.ts
...
async function checkForUpdates(): Promise<void> {
  ...
  try {
    ...
    if (version) {
      ...
      // For now, just log it
      console.error(`[MAI] Claude Code version: ${version}`);
    }
  } catch {
...
async function main() {
  try {
    ...
    // Output session info
    console.error(`[MAI] Session initialized: ${projectName}`);
    console.error(`[MAI] Time: ${getLocalTimestamp()}`);
