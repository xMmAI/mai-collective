#!/usr/bin/env bun
// $MAI_DIR/hooks/security-validator.ts
...
const ATTACK_PATTERNS = {
  ...
  // Tier 10: MAI-specific protection - Block
  paiProtection: {
    patterns: [
      /rm.*\.config\/pai/i,                        // Delete MAI config
      /rm.*\.claude/i,                             // Delete Claude config
      /git\s+push.*MAI.*public/i,                  // Push MAI to public
    ],
    action: 'block',
    message: '🚨 BLOCKED: MAI infrastructure protection triggered'
  }
};
