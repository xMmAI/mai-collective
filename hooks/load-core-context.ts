#!/usr/bin/env bun
// $MAI_DIR/hooks/load-core-context.ts
...
async function main() {
  try {
    ...
    const paiDir = process.env.MAI_DIR || join(homedir(), '.config', 'pai');
    ...
    if (!existsSync(coreSkillPath)) {
      ...
      console.error('[MAI] No CORE skill found - skipping context injection');
      process.exit(0);
    }

    // Read the skill content and expand env vars ($VAR → process.env.VAR)
    const rawContent = readFileSync(coreSkillPath, 'utf-8');
    const skillContent = rawContent.replace(/\$([A-Z_][A-Z0-9_]*)/g, (_, name) =>
...
MAI CORE CONTEXT (Auto-loaded at Session Start)
...
✅ MAI Context successfully loaded...`;
