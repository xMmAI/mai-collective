...
async function loadEnv(): Promise<void> {
  // Load from canonical location: $MAI_DIR/.env (single source of truth)
  // Falls back to legacy locations for backwards compatibility
  const paiDir = process.env.MAI_DIR || resolve(process.env.HOME!, '.config/pai');
  const envPaths = [
    resolve(paiDir, '.env'),
...
// MAI directory for documentation paths
const MAI_DIR = process.env.MAI_DIR || `${process.env.HOME}/.config/pai`;
...
MORE INFO:
  Documentation: ${MAI_DIR}/skills/Art/README.md
  Source: ${MAI_DIR}/skills/Art/Tools/Generate.ts
`);
  process.exit(0);
}
